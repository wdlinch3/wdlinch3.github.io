...
\begin{itemize}
  \item<1-> Critical superstrings live in 10 flat dimensions
  \item<2-> We live in 4 (almost flat) dimensions
  \item<3-> To use our tool, we need to reduce $10\to 4$ 
  \item<4->Two complementary possibilities:
  \item<5-> Compactification  
  \item<6-> Holography
  \end{itemize}
}
...
\pause
\begin{itemize}
 \item<1-> Compactifications on good string backgrounds without flux have moduli.
 \pause
 \item<2-> Example: Heterotic string on Calabi-Yau 3-fold has $N=1$ supersymmetry $\Rightarrow$ flat directions.
\end{itemize}
...
\begin{enumerate}
\pause
\item<1-> Hybrid strings
\pause
\item <2->Pure spinor strings
\end{enumerate}
}
...
\end{eqnarray}
\pause
Flux deformed background preserving D3 symmetry (Klebanov-Strassler)\pause
\begin{eqnarray}
...
\begin{itemize}
\item <1->This approach allows a worldsheet ({\it i.e.} string-theoretic) derivation of many spacetime effects of background fluxes
\pause
\item<2-> Can in principle be used to find all effects such as torsion, non-commutativity, {\it et cetera}
\pause
\item<3-> Suggests the validity of the flux background as a string background due to existence of worldsheet SCA (no breaking of worldsheet supersymmetry)
\pause
\item<4-> Allows computation of scattering amplitudes in non-trivial backgrounds
\pause
\item<5-> Existence of new type of mirror symmetry for heterotic strings?
\end{itemize}
\pause
...
\pause
\begin{itemize}
\item<1-> Compactification and relation to hybrid
\pause
\item<2-> Holographic backgrounds
\item \dots
\end{itemize}
\pause
Open problems
\pause
\begin{itemize}
\item<1-> Looks like gauge fixing of a superstring with some fermionic symmetry. {\em Which string ??}
\pause
\item<2-> Spectrum
\item \dots
\end{itemize}
